	...
	while(1){
	    if( ulTaskNotifyTake( pdTRUE, portMAX_DELAY ) ){					// Wait for receiving new serial data over USB
	    	while( USBBufferDataAvailable( &g_sRxBuffer ) ){
				nCharsRead += USBBufferRead( &g_sRxBuffer, &charBuffer[nCharsRead], CMD_PARSER_BUF_LEN-nCharsRead );
	...
	while(1){
		...
		for( i=0; i<OUT_WRITER_LIST_LEN; i++ ){
			if( outListPtr->i2cChannel<0 ){
				...
				ts_i2cTransfer( outListPtr->i2cChannel, outListPtr->i2cAddress, &outListPtr->bcmBuffer[bcmCycleCounter], 1, NULL, 0, NULL, NULL );
...
//	lowPower  = PWM value after  the pulse
	...
	if( outLocation.hwIndexType != HW_INDEX_I2C ) return;
	for( i=0; i<OUT_WRITER_LIST_LEN; i++ ){
...
//			Create new entry!
			...
			return;
		} else if( outListPtr->i2cChannel==outLocation.i2cChannel && outListPtr->i2cAddress==outLocation.i2cAddress ) {
...
int Cmd_SW(int argc, char *argv[]){
	...
	for( i=0; i<N_LONGS; i++ ){
		charsWritten += usnprintf( &outBuffer[charsWritten], REPORT_SWITCH_BUF_SIZE-charsWritten, "0x%08x,", g_SwitchStateDebounced.longValues[i] );
...
//	RUL 0 0x23 0x100 4 1 15 3 1 0 0
	...
	if( argc == 11 ){
		...
		if( outputDriverId.hwIndexType==HW_INDEX_INVALID || outputDriverId.hwIndexType==HW_INDEX_SWM ){
		...
		setupQuickRule( id, inputSwitchId, outputDriverId, triggerHoldOffTime, tPulse, pwmHigh, pwmLow, trigPosEdge, outOffOnRelease, levelTriggered );
		return(0);
	}
	return CMDLINE_TOO_FEW_ARGS;
}
