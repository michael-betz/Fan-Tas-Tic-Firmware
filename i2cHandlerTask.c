...
void i2CIntHandler0(void) { I2CMIntHandler(&g_sI2CInst[0]); }
void i2CIntHandler1(void) { I2CMIntHandler(&g_sI2CInst[1]); }
void i2CIntHandler2(void) { I2CMIntHandler(&g_sI2CInst[2]); }
void i2CIntHandler3(void) { I2CMIntHandler(&g_sI2CInst[3]); }
...
void ts_i2cTransfer( uint8_t channel, uint_fast8_t ui8Addr, const uint8_t *pui8WriteData, uint_fast16_t ui16WriteCount, uint8_t *pui8ReadData, uint_fast16_t ui16ReadCount, tSensorCallback *pfnCallback, void *pvCallbackData ){
//	Do a thread safe I2C transfer in background (add command to the i2c queue)
	if (channel > 3) return;
	taskENTER_CRITICAL();
	I2CMRead( &g_sI2CInst[channel], ui8Addr, pui8WriteData, ui16WriteCount, pui8ReadData, ui16ReadCount, pfnCallback, pvCallbackData );
	taskEXIT_CRITICAL();
}

uint8_t getSMrow(){
	uint8_t temp;
	temp =  GPIOPinRead( GPIO_PORTE_BASE, GPIO_PIN_3|GPIO_PIN_2|GPIO_PIN_1 )>>1;	//         ,2,1,0
...
//	Reset Shift register to 0x00
	...
	ROM_SysCtlDelay( SM_COL_DELAY_CNT );
	ROM_GPIOPinWrite( GPIO_PORTB_BASE, SM_COL_CLK|SM_COL_DAT, SM_COL_CLK|SM_COL_DAT );//Shift register clock = pos. edge
	ROM_SysCtlDelay( SM_COL_DELAY_CNT );
	ROM_GPIOPinWrite( GPIO_PORTB_BASE, SM_COL_CLK|SM_COL_DAT, 0 );
	ROM_SysCtlDelay( SM_COL_DELAY_CNT );
...
void i2cStartPCFL8574refresh(){
	...
	for( i=0; i<=MAX_PCLS_PER_CHANNEL-1; i++ ){
		ts_i2cTransfer( 0, 0x40+i, NULL, 0, g_SwitchStateSampled.switchState.i2cReadData[0], 1, i2cDoneCallback, &g_i2cReadStates[0][i] );
		ts_i2cTransfer( 1, 0x40+i, NULL, 0, g_SwitchStateSampled.switchState.i2cReadData[1], 1, i2cDoneCallback, &g_i2cReadStates[1][i] );
		ts_i2cTransfer( 2, 0x40+i, NULL, 0, g_SwitchStateSampled.switchState.i2cReadData[2], 1, i2cDoneCallback, &g_i2cReadStates[2][i] );
		ts_i2cTransfer( 3, 0x40+i, NULL, 0, g_SwitchStateSampled.switchState.i2cReadData[3], 1, i2cDoneCallback, &g_i2cReadStates[3][i] );
	}
}
		...
		if( g_SwitchStateToggled.longValues[i] ){
			...
			for( j=0; j<=31; j++ ){
				if( tempValue & 0x00000001 ){	//We found a bit that changed, report over serial USB
					...
					switchValue = (g_SwitchStateDebounced.longValues[i]>>j) & 0x00000001;
					charsWritten += usnprintf( &outBuffer[charsWritten], REPORT_SWITCH_BUF_SIZE-charsWritten, "%03x=%01d;", i*32+j, switchValue );
	...
	for( i=0; i<MAX_QUICK_RULES; i++ ){
		if( TF(QRF_ENABLED) ){									//If a rule is enabled:
			...
			if( TF(QRF_STATE_TRIG) ){							//	If it is currently triggered:
				...
				if( HWREGBITB( &g_SwitchStateToggled.charValues[bIndex], pinIndex ) || TF(QRF_LEVEL_TRIG) ){ // Check if pin toggled (skip if level triggered)
					if( pinValue==TF(QRF_TRIG_EDGE_POS) ){		//    Check if the edge matches
						...
						setPclOutput( currentRule->outputDriverId, currentRule->tPulse, currentRule->pwmHigh, currentRule->pwmLow );
					}
				}
			}
		}
...
void setupQuickRule( uint8_t id, t_outputBit inputSwitchId, t_outputBit outputDriverId,
					 uint16_t triggerHoldOffTime, uint16_t tPulse, uint8_t pwmHigh, uint8_t pwmLow,
...
//	uint32_t ticks;
	...
	setupQuickRule( 0, decodeHwIndex(0xFF), decodeHwIndex(0x100), 0, 0, 15, 15, true, true, false );
	vTaskDelay(1000);
